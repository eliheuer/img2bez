...
pub fn process(paths: &[BezPath], config: &TraceOptions) -> Vec<BezPath> {
    ...
    let constrain = |previous: &[BezPath], candidate| {
        fidelity::constrain(
            &reference,
            previous,
            candidate,
            config.cleanup_max_deviation,
        )
    };
    result = constrain(
        &result,
    ...
    if config.grid > 0 {
        ...
        result = constrain(
            &result,
            if config.structure_grid > config.grid {
    ...
    result =
        constrain(&result, result.iter().map(even::even_handles).collect());
    ...
    // directions).
    result = constrain(&result, result.iter().map(even::cap_handles).collect());

    if config.grid > 0 {
        result = constrain(
            &result,
            result.iter().map(snap::round_handles).collect(),
        );
    }
